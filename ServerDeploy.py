...
def create_directory_and_send_url(game_name):
    """
    ...
    try:
        ...
        # Send a message indicating that the game is up
        send_discord_message(f"The game {game_name} is up! URL: {url}")
        ...
        def wait_for_post():
            ...
            while directory_name in active_threads:
                ...
                if current_time - start_time >= 5:
                    ...
                    send_discord_message(f"The game {game_name} has been disconnected!")
                    break
...
def register():
    ...
    if game_name:
        directory_name = create_directory_and_send_url(game_name)
        if directory_name:
...
def upload_to_google_drive():
    ...
    if request.is_json:
        ...
        if 'file_content' in data and 'file_name' in data:
            ...
            # Write JSON data to a temporary text file
            temp_file_path = 'temp_file.txt'
            with open(temp_file_path, 'w') as temp_file:
                temp_file.write(file_content)

            # Upload the temporary file to Google Drive
            file_id = google_drive.upload_file(temp_file_path, file_name)
            if file_id:
                # Make the uploaded file public
                google_drive.make_public(file_id)
            ...
            return jsonify({'error': 'Invalid JSON format. Missing "file_content" or "file_name".'}), 400
    else:
